...
def test_version():
    ...
    bundle = kuber.create_bundle(
        kubernetes_version=latest.KUBERNETES_VERSION, bundle_name="foo"
    )
...
def test_add_resource():
    ...
    bundle.add(api_version="core/v1", kind="ConfigMap", name="foo")
...
def test_pop():
    ...
    popped_resource = bundle.pop(
        name=resource.metadata.name, kind=resource.kind, **resource.metadata.labels
    )
...
def test_render_json():
    ...
    bundle = kuber.from_directory(
        directory=MY_DIRECTORY, kubernetes_version=f"{version.major}.{version.minor}"
    )
