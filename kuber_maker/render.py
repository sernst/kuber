...
TEMPLATE_PATH_ROOT = os.path.realpath(
    os.path.join(os.path.dirname(__file__), "templates")
)

ENVIRONMENT = Environment(loader=FileSystemLoader(TEMPLATE_PATH_ROOT))
...
def _has_status(package_entities: typing.Dict[str, kuber_maker.Entity]) -> bool:
    """..."""
    return any(["status" in e.properties for e in package_entities.values()])


def _collection_status(entity: kuber_maker.Entity) -> dict:
    """..."""
    is_collection = (
        entity.class_name.lower().endswith("list") and "items" in entity.properties
    )

    return {"is_collection": is_collection, "items": entity.properties.get("items")}
...
def _to_status_class(class_name: str) -> str:
    ...
    return remaps.get(class_name, f"{class_name}Status")
...
ENVIRONMENT.filters["to_status_class"] = _to_status_class
...
def _get_container_entity(all_entities: kuber_maker.AllEntities) -> kuber_maker.Entity:
    """..."""
...
def _get_status_entity(all_entities: kuber_maker.AllEntities) -> kuber_maker.Entity:
    """..."""
    version = all_entities.version.replace(".", "_")
...
def render_package(package: str, all_entities: kuber_maker.AllEntities) -> str:
    ...
    data_types = set(
        [p.data_type.api_type for e in entities.values() for p in e.properties.values()]
    )
...
def render_root_package(version: str, spec: kuber_maker.ApiSpec):
    ...
    return render(
        "root-package.jinja2", spec=spec, version=version, dt=datetime.datetime.utcnow()
    )
